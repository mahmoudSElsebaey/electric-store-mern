  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-3xl text-teal-600 animate-pulse">
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <p className="text-3xl text-red-600 mb-6">{t("product_detail.error")}</p>
          <p className="text-xl text-gray-600 mb-8">{error || t("product_detail.not_found")}</p>
          <Link to="/store" className="bg-teal-600 hover:bg-teal-700 text-white px-8 py-4 rounded-xl text-lg font-bold">
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12" dir={isRTL ? "rtl" : "ltr"}>
      <div className="max-w-7xl mx-auto px-6">
        <Link to="/store" className="inline-flex items-center text-teal-600 hover:text-teal-800 text-lg font-medium mb-10">
          {t("product_detail.back_to_store")}
        </Link>
          ...
          <div className="p-10 lg:p-16 space-y-8">
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-6 text-lg text-gray-600">
                <span>{t("store.brand")}: <strong className="text-teal-600">{product.brand.name}</strong></span>
                <span>{t("store.category")}: <strong className="text-green-600">{product.category.name}</strong></span>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600">{t("store.reviews_count", { count: product.numReviews })}</p>
                </div>
              </div>

              <div className="border-t border-b border-gray-200 py-6">
                <div className="flex items-end gap-3">
                  <span className="text-5xl lg:text-6xl font-extrabold text-teal-600">
              ...
              <div className="border-t border-gray-200 pt-6">
                <h3 className="text-2xl font-bold text-gray-800 mb-4">{t("product_detail.description_title")}</h3>
            ...
            <button
              ...
              className={`w-full py-6 text-3xl font-extrabold rounded-2xl transition-all ${
                isOutOfStock
                  ...
                  : "bg-gradient-to-r from-teal-600 to-teal-700 text-white hover:from-teal-700 hover:to-teal-800 shadow-2xl"
              }`}
            >
        ...
        <div className="mt-16 max-w-5xl mx-auto">
          <h2 className="text-4xl font-extrabold text-center mb-12">{t("product_detail.reviews_title")}</h2>

          {state.isAuthenticated ? (
            <div className="bg-white rounded-2xl shadow-xl p-8 mb-12">
              <h3 className="text-2xl font-bold mb-6">{t("product_detail.add_review")}</h3>
              <form onSubmit={handleAddReview} className="space-y-8">
                <div>
                  <label className="block text-lg font-semibold mb-4">{t("product_detail.your_rating")}</label>
                  <StarRating
                    rating={reviewForm.rating}
                    size="xl"
                    interactive
                ...
                <div>
                  <label className="block text-lg font-semibold mb-4">{t("product_detail.your_comment")}</label>
                  <textarea
                    rows={5}
                    value={reviewForm.comment}
                    onChange={(e) => setReviewForm({ ...reviewForm, comment: e.target.value })}
                    className="w-full p-4 border rounded-xl focus:ring-4 focus:ring-teal-300"
                ...
                <button
                  ...
                  className="w-full bg-teal-600 hover:bg-teal-700 text-white py-5 rounded-xl font-bold text-xl disabled:opacity-60"
                >
          ...
          <div className="space-y-8">
            {product.reviews && product.reviews.length > 0 ? (
              ...
              <div className="text-center py-20 bg-white rounded-2xl shadow-xl">
                <p className="text-2xl text-gray-500">{t("product_detail.no_reviews_yet")}</p>
              </div>
            )}
          </div>
        </div>
